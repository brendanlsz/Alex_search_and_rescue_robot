...
int findColour() { //true for red and false for green
  digitalWrite(S2, LOW); 
  ...
  while (three--) {
    ...
    frequencyB += map(frequency, 150, 510, 255, 0);
  }
  ...
  while (three--) {
    ...
    frequencyR += map(frequency, 120, 590, 255, 0);
 ...
 // Serial.print(" Red: ");
  ...
  while (three--) {
    ...
    //Remaping the value of the frequency to the RGB Model of 0 to 255
    frequencyG += map(frequency, 160, 630, 255, 0);
  }
  frequencyG = frequencyG / 3.0;
  //Serial.print(" Green: ");
  //Serial.println(frequencyG);

  if (frequencyG >= 170 && frequencyR >= 170 && frequencyB >= 170) {//WHITE
    return 0;
  }
  else if(abs(frequencyR - frequencyG) <= 10)
  {//GREEN
   return 1;
  }
  else if (frequencyR - frequencyG >= 40 && frequencyR - frequencyB >= 40) {
    return 2; // RED
  }
  return 3;
}
...
void sendStatus()
{
  ...
  statusPacket.params[10] = findColour();
...
void sendColour() {
  ...
  colourPacket.params[0]  = findColour();
